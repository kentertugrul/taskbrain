...
const TaskItem: React.FC<{ task: Task }> = ({ task }) => {
  ...
  return (
    <div className="group bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-xl p-4 transition-all duration-200">
      <div className="flex items-center justify-between">
        <div className="flex items-start gap-4">
          <div className={`mt-1 w-4 h-4 rounded border-2 flex items-center justify-center cursor-pointer
            ${task.status === TaskStatus.DONE ? 'bg-emerald-600 border-emerald-600' : 'border-slate-600 hover:border-indigo-500'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="text-slate-600 hover:text-slate-300 p-2">
    ...
    <div className="max-w-5xl mx-auto relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Manage extracted tasks and decisions.</p>
        </div>
      ...
      <div className="space-y-3">
        {filteredTasks.length === 0 ? (
          <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-2xl bg-slate-900/50">
            <p className="text-slate-500">No tasks found in this view.</p>
          </div>
        ) : (
